                                    ...
                                    const bool Read)
{
	...
	FlashAddress += APP_START_ADDRESS;
	uint16_t len = SECTOR_SIZE_BYTES;
	uint32_t offset = 0;
	
	while (len > NVMCTRL_PAGE_SIZE) {
		/* Read or write one page data from/to flash */
		if (Read) {
			/* Read out the mapped block of data from the device's FLASH */
			nvm_read_buffer(FlashAddress, BlockBuffer + offset, NVMCTRL_PAGE_SIZE);
		} else {
			/* Check if it is first page of a row */
			if ((FlashAddress & 0xFF) == 0) {
				/* Erase row */
				nvm_erase_row(FlashAddress);
			}
			/* Write the data to flash */
			nvm_write_buffer(FlashAddress, BlockBuffer + offset, NVMCTRL_PAGE_SIZE);
		}
		/* Increment the address to be programmed */
		FlashAddress += NVMCTRL_PAGE_SIZE;
	...
	if (len > 0) {
		if (Read) {
			/* Read out the mapped block of data from the device's FLASH */
			nvm_read_buffer(FlashAddress, BlockBuffer + offset, len);
		} else {
			/* Write the data to flash */
			nvm_write_buffer(FlashAddress, BlockBuffer + offset, len);
		}
	}
